...
function lichessProfileHtml(user,ownAccount){
  const account=user.lichess;
  if(!account){
    return ownAccount?`<section class="lichess-card disconnected"><div><span class="lichess-mark">♞</span><div><h3>Lichess</h3><p>Подключите профиль, чтобы показать шахматные рейтинги.</p></div></div><a class="button secondary" href="/auth/lichess/login">Подключить Lichess</a></section>`:'';
  }
  const rating=value=>value??'—';
  const actions=ownAccount?`<div class="lichess-actions"><button class="button small secondary" data-action="sync-lichess">Обновить</button><button class="button small danger" data-action="disconnect-lichess">Отключить</button></div>`:'';
  return `<section class="lichess-card"><div class="lichess-heading"><span class="lichess-mark">♞</span><div><h3><a href="${escapeHtml(account.url)}" target="_blank" rel="noopener noreferrer">@${escapeHtml(account.username)}</a></h3><p>Lichess · обновлено ${formatDate(account.syncedAt)}</p></div></div><div class="lichess-ratings"><span><strong>${rating(account.rapid)}</strong><small>Rapid</small></span><span><strong>${rating(account.blitz)}</strong><small>Blitz</small></span><span><strong>${rating(account.classical)}</strong><small>Classical</small></span></div>${actions}</section>`;
}
function openProfile(id){
  ...
  if(!stats){
    ...
    document.querySelector('#profile-content').innerHTML=`<div class="profile-header"><span class="profile-avatar">${escapeHtml(initials(user))}</span><div><h2>${escapeHtml(visibleName(user))}</h2><p>${role}</p>${user.email?`<p>Почта ЛК: ${escapeHtml(user.email)}</p>`:''}</div></div>${lichess}${accountActions}`;
...
document.addEventListener('click',async event=>{
  ...
  if(event.target.closest('[data-action="sync-lichess"]')){const id=currentUser()?.id;document.querySelector('#profile-dialog').close();try{await mutate('/api/lichess/sync',{});openProfile(id);toast('Рейтинги Lichess обновлены')}catch(error){toast(error.message)}return}
  if(event.target.closest('[data-action="disconnect-lichess"]')){if(!confirm('Отключить профиль Lichess?'))return;document.querySelector('#profile-dialog').close();try{await mutate('/api/lichess/disconnect',{});toast('Профиль Lichess отключён')}catch(error){toast(error.message)}return}
